...
static int lomuto(void *array, int left, int right, size_t elem_sz,
    ...
    if(comp == str_cmp){
        for(int i = left+1; i <= right; i++){
            if(comp(*((char**)array + i), *((char**)array + left)) < 0){
                s++;
        ...
        char* pivot = (copy + (left * elem_sz));
        for(int i = left + 1; i <= right; i++){
            if(comp((copy + (i * elem_sz)), pivot) <= 0){
                s++;
                swap((copy + (s * elem_sz)), (copy + (i * elem_sz)), elem_sz);
            }
        }
        swap((copy + (s * elem_sz)), (copy + (left * elem_sz)), elem_sz);
        return s;
    }
}
