 ...
 * Date        : 6/04/2020
 * Description : A basic implementation of head, the command line utility that displays
...
int main(int argc, char *argv[]) {
    ...
    while((bytes_read = read(src, buf, BUFSIZE)) > 0){
        int index = 0;
        while(index < BUFSIZE && buf[index] != '\0' && numLines < line_count){
            int length = 0;
            ...
            numLines++;
            index++;
            if(write(STDOUT_FILENO, start, length) != length){
